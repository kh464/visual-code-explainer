...
8. Verify that the generated page is an interactive UI: sidebar navigation switches views without page scrolling, statistic cards open their target views, theme/share buttons respond, feature cards update the module drilldown/UML panel, UML zoom/reset/fullscreen controls respond, the feature demo button opens the matching demo, the module graph renders, runtime timeline renders, demo controls switch steps, autoplay works, tests render, and code-directory search filters rows.
